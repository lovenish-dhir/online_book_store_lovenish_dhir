...
if (isset($_SESSION['user_id']) &&
	...
	include "../dbConn.php";
    include "funcValidation.php";
    include "funcFileUpload.php";
          ...
          if (!empty($_FILES['book_cover']['name'])) {
		      if (!empty($_FILES['file']['name'])) {
		        ...
		        if ($book_cover['status'] == "error" || 
		            $file['status'] == "error") {
			    	$em = $book_cover['data'];
			    	header("Location: ../editBook.php?error=$em&id=$id");
			    	exit;
			    }else {
		           ...
		           $book_cover_URL = $book_cover['data'];
					...
					$res  = $stmt->execute([$title, $author, $description, $category,$book_cover_URL, $file_URL, $id]);
				     if ($res) {
						...
						header("Location: ../editBook.php?success=$sm&id=$id");
			            exit;
				     }else{
